...
From this, I can see that the query parameter for Eldercare is "eldercare".
...
The eldercare df will be processed into a sf dataframe
...
map_onemap <- tm_shape(mpsz19_shp) +
  ...
  tm_layout(main.title = "Eldercare (OneMap API)", 
...
map_data_gov <- tm_shape(mpsz19_shp) +
  ...
  tm_shape(eldercare_sf) +
  tm_dots(col='red', size = 0.5) +
  tm_layout(main.title = "Eldercare (Data Gov)", 
            main.title.position = ("center"))
...
```{r}
foodcourt <- st_read("data/geospatial/HawkerCentresKML.kml") %>%
  st_transform(crs = 3414)
```
...
```{r}
# test_sf <- st_read(dsn = "data/geospatial/TrainStation_Jul2024", layer = "RapidTransitSystemStation") %>%
#   st_transform(crs = 3414)

# test_sf <- st_read("data/geospatial/TrainStation_Jul2024/RapidTransitSystemStation.shp") %>%
#   st_transform(crs = 3414)
```
...
map_onemap <- tm_shape(mpsz19_shp) +
  ...
  tm_layout(main.title = "Parks (OneMap API)", 
            main.title.position = ("center"))
...
map_data_gov <- tm_shape(mpsz19_shp) +
  ...
  tm_dots(col='red', size = 0.2) +
  tm_layout(main.title = "Parks (Data Gov)", 
            main.title.position = ("center"))
...
# bbox_singapore <- getbb("Singapore")

# # Query for shopping malls
# shopping_malls <- opq(bbox = bbox_singapore) %>%
#   add_osm_feature(key = "shop", value = "mall") %>%
#   osmdata_sf()


# shopping_malls <- opq('Singapore') %>%
#   add_osm_feature(key = "shop", value = "mall") %>%
#   osmdata_sf()
```

```{r}
# singapore_boundary_query <- opq(bbox = getbb("Singapore")) %>%
#   add_osm_feature(key = "boundary", value = "administrative") %>%
#   add_osm_feature(key = "name", value = "Singapore") %>%
#   osmdata_sf()
# 
# # Extract the boundary as an sf object
# singapore_boundary <- singapore_boundary_query$osm_multipolygons
# 
# # Ensure the boundary exists and is non-empty
# if (nrow(singapore_boundary) > 0) {
#   # Run a refined query for shopping malls within the Singapore administrative boundary
#   shopping_malls <- opq(bbox = st_bbox(singapore_boundary)) %>%
#     add_osm_feature(key = "shop", value = "mall") %>%
#     osmdata_sf()
# 
#   # Filter shopping malls to only include those within the boundary
#   malls_points <- shopping_malls$osm_points %>%
#     st_intersection(singapore_boundary) %>%
#     filter(!is.na(name))
# 
#   malls_polygons <- shopping_malls$osm_polygons %>%
#     st_intersection(singapore_boundary) %>%
#     filter(!is.na(name))
# }
```
